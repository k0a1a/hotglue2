...
/*
// custom logging for hotglue.me
define('HG_ERROR_LOG', '/var/www-hotglue/logs/php-errors.log');

set_error_handler(function ($errno, $errstr, $errfile, $errline) {
    // respect @-suppression and the current error_reporting level
    if (!(error_reporting() & $errno)) return false;
    $line = sprintf("[%s] %s: %s in %s:%d\n",
        date('c'), hg_errlevel($errno), $errstr, $errfile, $errline);
    @error_log($line, 3, HG_ERROR_LOG);   // type 3 = append to file
    return false;   // false = also let PHP's normal handling run (fatal stays fatal)
});
...
register_shutdown_function(function () {
    ...
    if ($e && ($e['type'] & (E_ERROR|E_PARSE|E_CORE_ERROR|E_COMPILE_ERROR))) {
        @error_log(sprintf("[%s] FATAL: %s in %s:%d\n",
            date('c'), $e['message'], $e['file'], $e['line']), 3, HG_ERROR_LOG);
    }
});

error_log('test', 3, HG_ERROR_LOG);
*/
...
if (!defined('AUTH_METHOD')) define('AUTH_METHOD', 'db');   // was 'basic'
...
@define('SNAPSHOT_MIN_AGE', 0);			// auto- revisions are created every n seconds (zero to disable)
