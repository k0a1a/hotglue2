...
**Renamed 2026-09-16: the action is "object attributes", not "object properties".** The
...
what it points at — its attributes. Its button's tooltip is now "object attributes", and the
dialog carries the same words as its accessible name; `object-properties.spec.js` (which
keeps its filename) looks the button up as `/object attributes/`. Nothing else about it
changed: the modal, the service, the validation and the eleven tests are as this document
describes them.
